...
<span class="c">$ ghost upgrade pro</span>      <span class="m"># opens Stripe Checkout for the tier</span></div>
        <h3 class="doc-h3">Authorizing the CLI — the device flow</h3>
        <p class="doc-p"><code class="inl">ghost login</code> never asks for a password. It prints a short code (like <code class="inl">K7Q2-9FMX</code>) and opens <a href="/device" style="color:#7fb0ff;">ghost-memory.com/device</a> in your browser. Enter the code there, click <strong style="color:#dfe9ff;">Authorize this device</strong>, and the terminal picks up the token on its own — stored in <code class="inl">~/.ghost/ghost_token</code> (<code class="inl">0600</code>), never your Anthropic key. The code expires 10 minutes after you run the command; if it lapses, run <code class="inl">ghost login</code> again for a fresh one. <code class="inl">ghost whoami</code> confirms the connected account.</p>
